...
class OpdsController(
    ...
    fun getAllBooks(pageable: Pageable): Feed {
        ...
        val baseHref = "/opds/v2/books/all.json"

        // Self link
        links.add(Link(href = "$baseHref?page=${booksPage.page}&size=${booksPage.size}",
                       type = OPDS_JSON_MEDIA_TYPE,
                       rel = "self"))

        // First link
        if (!booksPage.isFirst) {
            links.add(Link(href = "$baseHref?page=0&size=${booksPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "first"))
        }
        ...
        if (booksPage.hasPrevious) {
            links.add(Link(href = "$baseHref?page=${booksPage.page - 1}&size=${booksPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "previous"))
        }
        ...
        // Last link
        if (!booksPage.isLast) {
            links.add(Link(href = "$baseHref?page=${booksPage.totalPages - 1}&size=${booksPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "last"))
        }
    ...
    fun getNewBooks(pageable: Pageable): Feed {
        ...
        val links = mutableListOf<Link>()
        val baseHref = "/opds/v2/books/new.json"
    ...
    fun getAuthorsIndex(pageable: Pageable): Feed {
        ...
        val links = mutableListOf<Link>()
        val baseHref = "/opds/v2/authors/index.json"

        // Self link
        links.add(Link(href = "$baseHref?page=${authorsPage.page}&size=${authorsPage.size}",
                       type = OPDS_JSON_MEDIA_TYPE,
                       rel = "self"))

        // First link
        if (!authorsPage.isFirst) {
            links.add(Link(href = "$baseHref?page=0&size=${authorsPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "first"))
        }

        // Previous link
        if (authorsPage.hasPrevious) {
            links.add(Link(href = "$baseHref?page=${authorsPage.page - 1}&size=${authorsPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "previous"))
        }

        // Next link
        if (authorsPage.hasNext) {
            links.add(Link(href = "$baseHref?page=${authorsPage.page + 1}&size=${authorsPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "next"))
        }
        ...
        // Last link
        if (!authorsPage.isLast) {
            links.add(Link(href = "$baseHref?page=${authorsPage.totalPages - 1}&size=${authorsPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "last"))
        }
    ...
    fun getAuthorBooks(@PathVariable authorId: UUID, pageable: Pageable): Feed {
        ...
        val links = mutableListOf<Link>()
        val baseHref = "/opds/v2/authors/$authorId/books.json"
    ...
    fun getSeriesIndex(pageable: Pageable): Feed {
        ...
        val links = mutableListOf<Link>()
        val baseHref = "/opds/v2/series/index.json"

        // Self link
        links.add(Link(href = "$baseHref?page=${seriesPage.page}&size=${seriesPage.size}",
                       type = OPDS_JSON_MEDIA_TYPE,
                       rel = "self"))

        // First link
        if (!seriesPage.isFirst) {
            links.add(Link(href = "$baseHref?page=0&size=${seriesPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "first"))
        }

        // Previous link
        if (seriesPage.hasPrevious) {
            links.add(Link(href = "$baseHref?page=${seriesPage.page - 1}&size=${seriesPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "previous"))
        }

        // Next link
        if (seriesPage.hasNext) {
            links.add(Link(href = "$baseHref?page=${seriesPage.page + 1}&size=${seriesPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "next"))
        }
        ...
        // Last link
        if (!seriesPage.isLast) {
            links.add(Link(href = "$baseHref?page=${seriesPage.totalPages - 1}&size=${seriesPage.size}",
                           type = OPDS_JSON_MEDIA_TYPE,
                           rel = "last"))
        }
    ...
    fun getSeriesBooks(@PathVariable seriesId: UUID, pageable: Pageable): Feed {
        ...
        val links = mutableListOf<Link>()
        val baseHref = "/opds/v2/series/$seriesId/books.json"
