      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-6 animate-slide-in-left">
          <button 
            ...
            className="p-4 bg-white text-gray-400 hover:text-emerald-600 border border-gray-200 rounded-2xl transition-all shadow-sm hover:shadow-md active:scale-95 group"
          >
            <FaArrowLeft size={20} className="transform group-hover:-translate-x-1 transition-transform" />
          </button>
          
          <div className="w-20 h-20 bg-emerald-600 text-white rounded-3xl shadow-xl shadow-emerald-100 flex items-center justify-center relative overflow-hidden group">
            <div className="absolute inset-0 bg-white/20 transform -skew-x-12 translate-x-full group-hover:-translate-x-full transition-transform duration-1000"></div>
            <FaUserTag size={32} />
          </div>
          
          <div className="flex-grow">
            <p className="text-xs text-emerald-600 font-black uppercase tracking-widest mt-1">{t('authorDetails.entityLabel', 'Author Entity')}</p>
        ...
        <div className="flex gap-3">
          <Link 
            to={`/authors/${id}/edit`} 
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase tracking-widest py-3 px-8 rounded-2xl shadow-xl shadow-emerald-100 transition-all flex items-center gap-2 transform hover:-translate-y-1 active:scale-95"
          >
            <FaEdit />
            {t('common.edit')}
          </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/50 flex flex-col gap-6 relative overflow-hidden group">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-2"><FaCalendarAlt className="text-emerald-400" /> {t('authorDetails.birthDate', 'Birth Date')}</p>
            ...
            <div className="border-t border-gray-50 pt-6">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2"><FaInfoCircle className="text-emerald-400" /> {t('authorDetails.bio', 'Short Bio')}</p>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="flex items-center justify-between px-4">
            <h2 className="text-xs font-black text-gray-400 uppercase tracking-[0.3em] flex items-center gap-2"><FaBook className="text-emerald-400" /> {t('authorDetails.booksByAuthor', 'Books by Author')}</h2>
            <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">{author.books?.length || 0}</span>
          </div>
            ...
            {author.books && author.books.length > 0 ? (
              author.books.map((book) => (
                <Link key={book.id} to={`/book/${book.id}`} className="block group/book">
                  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-xl shadow-gray-100/50 flex items-center justify-between group-hover/book:border-emerald-200 transition-all group-hover/book:-translate-y-1">
                    <div className="flex items-center gap-4">
                      <div className="p-3 rounded-xl bg-emerald-50 text-emerald-600 group-hover/book:bg-emerald-600 group-hover/book:text-white transition-all">
                        <FaBook size={16} />
                      </div>
                      <span className="text-sm font-black text-gray-800 tracking-tight">{book.title}</span>
                    </div>
                    <FaChevronRight className="text-gray-300 group-hover/book:text-emerald-600 transition-colors" />
                  </div>
                </Link>
              ))
            ) : (
